Shader "Custom/110-119/114_02_Fresnel_Frag" 
{
	Properties
    {
    ...
    SubShader
    {
        Pass 
        {
            ...
            fixed4 frag(vertexOuput i) : SV_TARGET  
            {
                ...
                fixed3 specularColor = SpecularBlinnPhong(worldNormalAtPixel, lightDir, worldSpaceViewDir, attenuation, 1, _SpecularPower, _SpecularFactor);
                ...
                float fresnel = 1.0 -saturate(dot(worldSpaceViewDir, worldNormalAtPixel));
                fresnel = smoothstep(1.0 - _FresnelWidth, 1.0, fresnel);
                
                fixed3 surfaceColor = mainTexCol * diffuseColor + specularColor + ambientColor;
                surfaceColor = lerp(surfaceColor, surfaceColor * reflectionColor, fresnel);
